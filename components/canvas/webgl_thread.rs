...
impl WebGLImpl {
    ...
    pub fn apply<Native: NativeGLContextMethods>(ctx: &GLContext<Native>, command: WebGLCommand) {
        match command {
            ...
            WebGLCommand::GetParameterBool(param, sender) => {
                let mut value = 0;
                unsafe {
                    ctx.gl().get_boolean_v(param as u32, ref_slice_mut(&mut value));
                }
            ...
            WebGLCommand::GetParameterInt(param, sender) => {
                let mut value = 0;
                unsafe {
                    ctx.gl().get_integer_v(param as u32, ref_slice_mut(&mut value));
                }
            ...
            WebGLCommand::GetParameterFloat(param, sender) => {
                let mut value = 0.;
                unsafe {
                    ctx.gl().get_float_v(param as u32, ref_slice_mut(&mut value));
                }
                sender.send(value).unwrap()
            }
            ...
            WebGLCommand::GetProgramParameterBool(program, param, sender) => {
                let mut value = 0;
                unsafe {
                    ctx.gl().get_program_iv(program.get(), param as u32, ref_slice_mut(&mut value));
                }
                sender.send(value != 0).unwrap()
            }
            ...
            WebGLCommand::GetShaderParameterBool(shader, param, sender) => {
                let mut value = 0;
                unsafe {
                    ctx.gl().get_shader_iv(shader.get(), param as u32, ref_slice_mut(&mut value));
                }
                sender.send(value != 0).unwrap()
            }
            ...
            WebGLCommand::GetVertexAttribBool(index, param, sender) => {
                ...
                let mut max = 0;
                unsafe {
                    ctx.gl().get_integer_v(gl::MAX_VERTEX_ATTRIBS, ref_slice_mut(&mut max));
                }
                let result = if index >= max as u32 {
                    Err(WebGLError::InvalidValue)
                } else {
                    let mut value = 0;
                    unsafe {
                        ctx.gl().get_vertex_attrib_iv(index, param as u32, ref_slice_mut(&mut value));
                    }
                    Ok(value != 0)
                };
                sender.send(result).unwrap();
    ...
    fn active_attrib(
        ...
        let mut max = 0;
        unsafe {
            gl.get_program_iv(program_id.get(), gl::ACTIVE_ATTRIBUTES, ref_slice_mut(&mut max));
        }
        let result = if index >= max as u32 {
            Err(WebGLError::InvalidValue)
        } else {
    ...
    fn active_uniform(gl: &gl::Gl,
        ...
        let mut max = 0;
        unsafe {
            gl.get_program_iv(program_id.get(), gl::ACTIVE_UNIFORMS, ref_slice_mut(&mut max));
        }
        let result = if index >= max as u32 {
            Err(WebGLError::InvalidValue)
        } else {
